...
def send_password_reset_email(to_email: str, otp_code: str) -> bool:
    ...
    if not smtp_user or not smtp_pass:
        logger("EMAIL", "SMTP credentials not configured — cannot send password reset OTP", level="ERROR")
        return False
    ...
    try:
        ...
        logger("EMAIL", f"Password reset OTP sent to {to_email}", level="INFO")
        return True

    except Exception as e:
        logger("EMAIL", f"Failed to send password reset email to {to_email}: {str(e)}", level="ERROR")
        return False
