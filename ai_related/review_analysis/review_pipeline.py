...
from ai_related.review_analysis.review_specificity import (
    fetch_project_vocabulary,
    measure_specificity,
)
...
async def run_post_review_pipeline(review_id: str, is_retry: bool = False) -> None:
    ...
    try:
        ...
        # LLM - text-rating disagreement and sentiment. Both are genuine ensemble
        # members: sentiment_score/label come from the Cardiff model outright, and
        # sentiment_mismatch needs both it and the LLM to agree.
        #
        # predict_authenticity used to run here too. It no longer does: measured on
        # this domain it scored templated reviews MORE genuine than real ones
        # (0.011-0.064 raw P(fake) against 0.080-0.119), so its advisory reason was
        # worse than silence. See blend_authenticity. Authenticity, fake and coerced
        # are decided by the LLM alone.
        #
        ...
        # Step 6c: Component 5, and the only check here that does not go through a
        # model at all - does the review name anything from this project? Arithmetic
        # over the contract's own vocabulary; see review_specificity.py for why the
        # LLM's judgement alone was not enough.
        specificity = measure_specificity(
            review_text=overall_comment,
            answer_text=client_answer,
            project_vocabulary=fetch_project_vocabulary(
                review["contract_id"], extra=[written.get("ai_question")]
            ),
        )
        ...
        overall_pass = compute_overall_pass(
            ...
            lacks_project_specificity=specificity["is_generic"],
        )
