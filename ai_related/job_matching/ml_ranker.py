...
Why ML is still needed despite Stage 1 + Stage 2

Stage 1 (cosine similarity) is a strong broad-retrieval signal but treats the job
posting as a single semantic object — it cannot compare budget numbers, detect
experience mismatches, or weight skill proficiency. A job with three unrelated roles
(backend engineer + financial manager + UI designer) produces an averaged embedding
that gives moderate cosine similarity to every specialist; the ML ranker correctly
identifies which specialist fits which role by computing skill features on the
best-matching role only (see per-role logic in _compute_job_features).

Stage 2 drops obvious mismatches but outputs an unordered list. Without re-ranking,
a job that perfectly fits the freelancer's budget and experience level ranks the same
as one where the budget is triple their rate — cosine similarity alone cannot
...
SHAP match_reasons give the homepage card an explanation layer absent from any
streaming recommendation service: freelancers on this platform need to know WHY
a job was surfaced before investing time writing a proposal. This is a deliberate
product feature, not a metric artefact.

Why homepage ML score differs from job-detail RAG score (intentional)

They answer different questions at different points in the user journey:

  Homepage ML score   — a RANKING signal. Calibrated probability (0–100) trained
                        to separate good fits from poor fits across 13
                        features. Answers: "Among all available jobs, how does this
                        rank for you?" Computed in ~0.6 ms for 100 candidates.

  Job-detail RAG score — a per-role ADVISORY signal. Anchored to required skill
                        coverage (hard ceiling = coverage_pct + 25 so the LLM
                        cannot wildly overscore a poor match), then adjusted by an
                        LLM reading past contract text, reviews, and portfolio
                        items. Answers: "Should you apply to this specific role,
                        and what are your concrete skill gaps?" Takes 5–30 s.

A job can rank high in ML (strong semantic match, budget fits, experience aligns)
yet score lower in RAG (missing 3 specific required skills for a particular role).
That divergence is informative, not a bug: it signals "this job is contextually
relevant to you but the role requirements are tighter than your skill set."
Hiding that divergence by using one score for both surfaces would mislead users.

Key fixes vs the original version

Per-role best-match (v2): all five skill features and the budget comparison are
now computed against the single best-matching role rather than a flat union of all
roles in the job post. This fixes a systematic bias where a backend engineer
applying to a 3-role job (backend + finance + design) would receive a 33% skill
overlap instead of 100% for their relevant role.

has_portfolio now counts completed contracts as past-work evidence — consistent
with how portfolio_relevance already unions portfolio_embedding and
contract_embedding when computing the semantic similarity feature.

Features (13 model features, all non-NaN for every freelancer)
  ...
  model — they were dropped from FEATURE_COLS due to low importance (1.3 each)
  and are profile-completeness signals rather than job-fit signals.

Cold-start fairness: portfolio_relevance falls back to max(0, cosine_sim - 0.10)
when the freelancer has no past-work evidence. This prevents new freelancers with
strong skills from being penalised twice.

Removed vs older versions: performance_score, success_rate_hist, is_cold_start.
Those were freelancer-quality penalties, not job-fit signals.
"""
