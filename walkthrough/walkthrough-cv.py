...
Flow:
  ...
  4.  Display full analysis results:
        - Similarity score (SentenceTransformer cosine, CV vs. profile)
        - Skill coverage   (regex word-boundary matching)
        - ATS compliance   (rule-based 0–100)
        - Overall scoring  (good / enough / bad)
        - ATS flags        (missing sections, no email, clichés, etc.)
        - LLM recommendations (Ollama gemma4 / Gemini fallback)
...
def run(base_url: str):
    ...
    step("Register a new freelancer account")
    ...
    for name, level in want_skills.items():
        ...
        if sk:
            ...
            }, token=token, expected={200, 201}, label=f"      Add skill: {name} ({level})")
    ...
    step("POST /cv_upload — extract text → embedding → ATS check → LLM recommendations")
    ...
    print(f"    │    (model: SentenceTransformer fine-tuned for CV matching)")
    ...
    if cov is not None:
        ...
        print(f"    │    (matching: regex word-boundary, case-insensitive)")

    # ATS score
    print(f"    │")
    print(f"    │  ATS Compliance    : {ats}/100")
    print(f"    │  [{_bar(ats/100)}]")
    print(f"    │  → Rule-based scoring: section presence, contact info, word count, content quality")
    print(f"    └────────────────────────────────────────────────────────")

    # Scoring logic
    print(f"""
    ┌─ SCORING LOGIC ───────────────────────────────────────
    │  good   : similarity ≥ 0.82  AND  coverage ≥ 0.65  AND  ats ≥ 70
    │  enough : similarity ≥ 0.70  AND  coverage ≥ 0.40  AND  ats ≥ 50
    │  bad    : below thresholds, or ats < 50
    │
    │  Current values:
    │    similarity = {sim:.4f}  (good: ≥0.82 | enough: ≥0.70)
    │    coverage   = {f"{cov:.4f}" if cov is not None else "N/A     "}  (good: ≥0.65 | enough: ≥0.40)
    │    ats_score  = {ats}/100   (good: ≥70  | minimum: ≥50)
    └────────────────────────────────────────────────────────""")
    ...
    if matched:
        ...
        print(f"    │  ✗ No profile skills were found in the CV text")
    if missing:
        print(f"    │")
    ...
    print(f"""
    ┌─ ATS SCORING BREAKDOWN (rule-based) ──────────────────
    │  Section presence   : max 40 pts
    │    [summary/profile +10, skills +10, experience +10, education +10]
    │  Contact info       : max 15 pts  [email +8, phone +7]
    │  Word count         : max 15 pts  [ideal range: 300–800 words]
    │  Content quality    : max 30 pts
    │    [quantifiable metrics +10, no clichés +10, dates present +10]
    └────────────────────────────────────────────────────────""")

    # Recommendations
    print(f"\n    ┌─ RECOMMENDATIONS ({len(recs)} item(s)) ─────────────────────")
    if recs:
        for i, rec in enumerate(recs, 1):
            rec_str = str(rec)
            lines   = [rec_str[j:j+100] for j in range(0, len(rec_str), 100)]
            print(f"    │  {i}. {lines[0]}")
            for ln in lines[1:]:
                print(f"    │     {ln}")
            if i < len(recs):
  ...
  │  ATS Score   : {ats}/100                                                │
  │  ATS Flags   : {len(flags)} issue(s)                                           │
  │  Recs        : {len(recs)} recommendation(s)                                  │
