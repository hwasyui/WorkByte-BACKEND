"""
Toxicity Detection Walkthrough — RoBERTa ML model (keyword fallback).
...
Three scenarios:
  ...
  Scenario 1 — CLEAN JOB
    ...
    Background toxicity scan finds nothing. No moderation flag.
    Job stays active.
...
Usage:
    docker exec -it capstone-backend python /app/walkthrough/walkthrough-toxicity.py
Or locally:
    python walkthrough/walkthrough-toxicity.py
"""
...
def run():
    ...
    print("\n" + "=" * 65)
    print("  Capstone API — Toxicity Detection Walkthrough")
    print("=" * 65)
    ...
    print("\n  Background toxicity scan triggered — API returned immediately.")
    ...
    if job1.get("status") == "active" and flag1 is None:
        print("  PASS — clean job active, no toxicity flag created")
    else:
    ...
    print("  Background toxicity scan running (asyncio task) ...")
  ...
  Detection flow:
    ...
    is_flagged = True       →  toxicity_queue record inserted (status=pending)
