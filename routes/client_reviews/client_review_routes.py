...
async def _reject_client_review_text_if_harmful(
    client_review_id: str,
    reviewer_id: str,
    freelancer_answer: str,
    overall_comment: str,
):
    """Same content-safety gate as _reject_review_text_if_harmful on the freelancer
    side (review_routes.py) - this review names a specific client and the client has
    no say in whether it goes live, so reject outright on the same scan rather than
    the save-then-block pattern the AI-analysis pipeline uses for a different concern."""
    combined = " ".join(t for t in (freelancer_answer, overall_comment) if t and t.strip())
    if not combined.strip():
        return None
    try:
        harm_result = await scan_harmful_text_with_ml_fallback(combined, timeout=ML_SCAN_TIMEOUT_BLOCKING_SECONDS)
    except Exception as e:
        logger("CLIENT_REVIEW", f"Client-review text scan errored, failing open (allowing save): {e}", level="WARNING")
        return None
    if not harm_result["is_flagged"]:
        return None
    detected_labels = harm_result.get("detected_labels", [])
    labels = [_CLIENT_REVIEW_LABEL_NAMES.get(l, l) for l in detected_labels]
    logger("CLIENT_REVIEW", f"Blocked client review submission, labels={detected_labels}", level="WARNING")
    insert_harmful_text_queue_entry("client_review", client_review_id, reviewer_id, combined, harm_result)
    return {
...
async def submit_client_review(
    ...
    try:
        ...
        rejection = await _reject_client_review_text_if_harmful(client_review_id, review["reviewer_id"], freelancer_answer, overall_comment)
        if rejection:
